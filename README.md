...
| カテゴリ | 内容 |
|---------|------|
| エージェント (5) | コードレビュー、設計レビュー、機能実装(設計成果物＋TDD: feature-implementer)、ドキュメント整合性検証、E2Eテスト実装(e2e-engineer) |
| スキル | 機能定義(要件＋クリティカル設計)、チケット作成、並列実装、TDD実装、技術負債チェック、プロジェクト初期設定、E2Eテスト作成、E2Eテストシナリオ解説＋独立検証、E2Eテスト実行、動作確認(ウォークスルー)、PRレビュー対応、PRマージ、Conventional Commits、PRセルフレビュー、品質ゲートチェック |
