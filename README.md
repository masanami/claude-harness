...
| カテゴリ | 内容 |
|---------|------|
| エージェント (7) | コードレビュー、設計レビュー、新規実装(TDD)、既存機能変更、ドキュメント整合性検証、E2Eテスト実装(e2e-engineer)、E2E独立検証(e2e-verifier) |
| スキル | 自律開発(全自律)、並列実装、技術負債チェック、要件定義、プロジェクト初期設定、E2Eテスト作成、E2Eテスト実行、動作確認(ウォークスルー)、PRレビュー対応、PRマージ、Conventional Commits、PRセルフレビュー、品質ゲートチェック、チケット作成 |
...
| スキル | 使い方 | 説明 |
|--------|--------|------|
| `/create-e2e` | `/create-e2e [Issue/PR/機能]` | 仕様ベースのE2Eテスト実装（非対話・トレーサビリティ・解説生成付き） |
| `/walkthrough` | `/walkthrough [Issue/PR/機能]` | AIがHeaded Playwrightで動作確認（ユーザーは観察して承認） |
| `/run-e2e` | `/run-e2e [ファイル名]` | E2Eテストの実行と結果分析（trace等のエビデンス保持） |
