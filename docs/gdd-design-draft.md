...
4. **機能仕様の退役**: `docs/features/` の各ドキュメントについて、(a) まだ真である約束は台帳へ（手順2で大半はテスト経由で拾われている想定）、(b) 恒常的に参照すべきクリティカル設計決定（認可モデル・スキーマ方針等）は ADR（`docs/adr/`）または恒常設計ドキュメントへ昇格、(c) 残りはファイルごと削除する。
   ...
   - **削除の前に、対象の全件について「ADR 昇格の要否判定」を必須ステップとして行う**（`/create-adr promote`。Issue #162 で受け皿を整備した際の追加決定）。仕様の削除は不可逆であり、ここが恒常的な設計決定を拾う最後の機会であるため、判定結果が「昇格不要」でもよいが判定自体は省略しない。何を昇格させるかの線引き（3条件）・ADR の置き場・命名・採番・構成・改訂の規約は、**戦略ドキュメント「4.3 恒常的な設計決定の記録（ADR）」が正本**。
   - **退役 PR のレビュー観点は「削除して良いか」ではなく「昇格させるべき決定が残っていないか」**とし、PR 本文に全件の片付け先と、昇格しなかったものの判定理由を載せる（「判定していない」と「判定した結果、不要だった」を区別可能にするため）。
...
4. **台帳ドラフト生成**: `public` を §2.2 書式で **`docs/guarantees.draft.md`** に書き出す（**D-17**: 正本 `docs/guarantees.md` には書かない。既存の正本が未裁可の再実行結果で上書きされる事故を防ぐ。ドラフトパスが既に存在する場合は上書きせず失敗し、人間に前回ドラフトの処置を促す）。**ドラフトには ID を振らず**（見出しは約束文のみ）、`宣言元` は `裁可待ち` と記載する。ID は裁可 PR で初めて採番する（**P2 実装時の改訂**: 当初案は枝番のみの仮置き ID（`G-?-1` 等）だったが、正本である戦略ドキュメント 5.3 の「仮 ID → 正式化という2段階を持たない」「採番できる場所を1つに絞る」という規約と衝突するため、仮 ID を廃した。PR #163 レビュー指摘）。`uncertain` は台帳に入れず「要人間判定」節として**報告側**に出す。
...
- **D-10: GDD期でも新機能開発には define-feature を使い続ける。** 機能仕様ドキュメントは廃止せず「短命な作業文書」と再定義する（並列実装のためのクリティカル設計集約という価値はフェーズ非依存のため）。リリース後に §3.3 手順4 の退役手順で台帳・ADR へ吸収して削除する。
  - > **【D-10 の「短命な作業文書」は撤回された・2026-08-21】** 既定フローでは**機能仕様を機能の追加・変更のたびに保守し、リリース後に破棄しない**（`docs/adr/0002-gdd-not-adopted-salvage-instruments.md` の「改訂（2026-08-21）」が D-10 を名指しで「倒さない」としている。既定フローの正本は `docs/ai-driven-development-strategy.md` 4.4）。D-10 の前半（GDD期でも define-feature を使い続ける／クリティカル設計集約の価値はフェーズ非依存）は撤回されていない。
...
| D-10 | GDD期も新機能は define-feature を使う（仕様は短命文書と再定義。**「短命文書」の部分は ADR 0002 の改訂で撤回**。§5.2 の注記参照） | クリティカル設計集約の価値はフェーズ非依存 |
