...
要件は開発者が定義し、**設計以降はエージェントが自律的に実行する**。**クリティカルな設計の意思決定（DB スキーマ・認証/認可・外部連携 等）は要件定義の段階でまとめて行い**、実装フェーズに人間ゲートを残さないことで、複数チケットの並列実装を中断なく走らせる。

```mermaid
flowchart TD
    A["0. 要件定義 + クリティカル設計決定 〔開発者+エージェント〕<br/>要件・完了条件 + DB/認証/外部連携 等の設計方針をチケットに記述"]
    B["1. 設計 〔エージェント〕<br/>既存コード調査・設計方針・影響範囲・実装計画の決定<br/>※クリティカル決定は要件チケットに従う"]
...
- **要件定義 + クリティカル設計決定（チケット）**: 「何を作るか」と「DB設計・スキーマ変更、認証/認可、外部連携など、誤ると手戻りが大きい設計の方針」を人間が決める。これらを**要件定義の段階でまとめて決定**することで、実装フェーズに人間ゲートを残さず、複数チケットの並列実装を中断なく回せる（→ 3.4）。
...
クリティカル設計の意思決定は**要件定義の段階でまとめて行う**（`/define-requirements` の責務）。実装フェーズに人間ゲートを置かないことで、複数チケットの並列実装を中断なく回せる。

要件チケットには「クリティカル設計決定」セクションを設け、認可モデル・暗号化方式・スキーマ方針・外部API契約などの方針を記録する。後続の `/design-feature` および `feature-implementer` は、その決定に従って実装する（独自判断で逸脱しない）。

#### レビューの担い手

| レビュー | 担い手 | タイミング | 要否 |
|---------|--------|------------|------|
| クリティカル設計の意思決定 | 人間 | **要件定義時** | **必須**。脅威モデル・影響範囲・不可逆性など、人間の判断が要る領域 |
| コードレビュー | AIエージェント（専用レビューエージェント） | 実装中 | **必須** |
| コードレビュー | 人間 | PRレビュー時 | 必須としない。重要度が極めて高い場合や、AIが懸念を上げた場合にエスカレーション |

> **理由**: クリティカル箇所でも、コードの精査はAIの方が安定して網羅的なことが多い。希少な人的リソースは**要件定義時の設計判断**に集中させ、コードはAIレビュー＋テストで多層に担保する。
