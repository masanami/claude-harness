...
- **一覧が照合するのは「コマンド名」であり、名前から実体への写像は `PATH` が行う。** ランナーは検証時に `command -v` で解決した**絶対パス**へ argv[0] を固定するため、検証した実体と起動する実体は必ず一致する。また `PATH` に相対エントリ（`.` や cwd 相対のディレクトリ）がある場合の解決は**拒否**する（作業ツリーへ書き込めるだけで許可名を差し替えられてしまうため）。しかし**`PATH` そのものの汚染は防げない**——`PATH` 上のディレクトリへ実行ファイルを置ける主体は、`npm` や `jest` といった許可名を別のバイナリへ差し替えられる。
