...
| `claude-harness-run read-plugin-doc <plugin相対パス>` | **届く**（`Bash(claude-harness-run:*)` の1行で allowlist 済み） |
...
- **定型注記の不変コア節** ((ix)(x)): 「スクリプトの実行形（重要）」「参照ファイルの読み出し（重要）」の各コピーに、規約が機能するために欠かせない節が揃っていること。**正本とのバイト一致は要求しない**（呼び出し箇所ごとに正当に異なる部分を潰すと回帰を作るため。(b) 参照）。実際に `agents/feature-implementer.md` のコピーが allowlist 根拠・プレースホルダ・恒久解の3節を落としていたのを本検査で検出した
  - こちらも**検出器自体の自己検査**を持つ（完全形で欠落を報告しないこと・既知のドリフト形を検出できることの両方を確認する）
