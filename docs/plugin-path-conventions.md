...
**正本コメントは純粋なポインタにする。** 実行時ファイルの HTML コメントがモデルへ配送されないことは、**保証された挙動ではない**（`CLAUDE.md` の自動注入では除去されるが、エージェント定義の本文はシステムプロンプトになる）。したがって「配送されない」側に賭けず、**配送されても失われる規範が無い**側で成立させる——規範は常にコメントの直前にインラインで書き切り（→ (f)）、コメントには正本の所在だけを書く。`scripts/tests/test-runtime-text-conventions.sh` (R-9) がこの2点（コメントがポインタの書式ちょうどであること／コメントを除いても判定軸が読み取れること）を固定する。
...
| 正本コメントの配送可否の実測（Issue #206） | `agents/*.md`・`SKILL.md` の HTML コメントが実際に配送されるかをプローブで確認し、慣行（37箇所）の扱いを確定する | 37箇所に及ぶ慣行の是非は本節の射程外。配送されても規範が失われない形（上記 R-9）で先に成立させてある |
...
- 本文書（`docs/plugin-path-conventions.md`）を実行時ファイルから示す場合も、装飾的な出典として文中に埋め込まない。開発者向けの1行コメントとして **HTML コメントで残す**（コメントがモデルへ配送されないことは保証された挙動ではないため、コメントには**正本の所在だけ**を書き、規範は必ずコメントの外に書き切る → (h)）:
