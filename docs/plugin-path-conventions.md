...
- **正本コメントの隣接重複** ((xii)): 定型注記末尾の `<!-- 正本: ... -->` が直前の行と同一のまま2行並んでいないこと。同一ファイル内に複数の正本コメントが在ること自体は正常（注記ブロックごとに1つ）で、違反は隣接した重複だけ。注記本体だけを差し替える一括置換は、置換テンプレート側にも正本コメントを含めていると**既存のコメント行を残したまま**もう1行足してしまう（本規約の移行時に10箇所中9箇所で発生）。比較は bash の文字列比較（バイト厳密）で行う — 正本コメントは非 ASCII を含むため、macOS 標準 awk の `==` は使わない
