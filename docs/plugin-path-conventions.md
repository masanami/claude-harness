...
本文書はパス参照メカニズムごとの規約を1箇所に集約する正本。`scripts/README.md` は scripts/ 配下の実装規約（jq前提・出力規約・テスト方針等）のみを扱い、プラグイン内ファイル参照のパス解決はここを参照する。Dynamic Workflow は全廃済み（Issue #106）。Workflow ランタイムに関する実機検証記録は `docs/adr/2026-08-01-dynamic-workflow-historical-record.md` に移設済み（Issue #126）。
