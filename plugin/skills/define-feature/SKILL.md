...
model: opus
# effort: 要件・クリティカル設計の意思決定を担う最上流のため high（Opus 5.5 の既定 medium より一段上）。
effort: high
---
...
- 手順1の「設計ドキュメントの探索」で特定した関連設計ドキュメントのパス（サブエージェントがコードベースと合わせて Glob/Grep/Read で調査できるように）。特定できていない場合は「未特定」と明示し、サブエージェント側に手順1と同じ探索手順を指示する
- 報告フォーマットの指示:
...
全サブエージェントの報告が揃ったら、メイン（opus/high）エージェントはヒアリングの全文脈を踏まえて各サブエージェントの `recommended`（推奨案）を検証し、必要なら修正する。各サブエージェントの報告をそのまま次のステップの提示に流し込まない。複数領域が相互作用する場合（例: 機密データ×DBスキーマ）は、各サブエージェントの調査結果をメインが統合してから次のステップへ渡す。
