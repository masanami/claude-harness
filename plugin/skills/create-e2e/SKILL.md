---
name: create-e2e
description: "仕様（チケットの完了条件・受入基準）からE2Eテストを実装する（非対話）。Triggers on: '/create-e2e', 'E2Eテストを書いて', 'E2Eテストを追加して'"
...
> 画面を操作した**動作確認**が必要な場合は `/demo`（AIがブラウザを操作し、人間は観察して承認する）を使う。本スキルは動作確認ではなく、テストの設計・実装に専念する。
