...
- ディレクトリ構成: （略。検出結果の確認用であり、`CLAUDE.md` には書き出しません）
...
生成する `CLAUDE.md` には、**コードベースから読み取れないもの**と**ツール既定と異なる規約**だけを書きます（プロジェクト概要・技術スタック・ディレクトリ構成は、Claude がコードを読めば分かるため書きません）。次の点だけ教えてください。

1. **品質ゲート**（PR 前に必ず通すコマンドと閾値。例: `npm run lint && npm test`、カバレッジ 80% 以上）
2. **モックの方針**（何をモックし、何をモックしないか）
3. **新規ファイルの置き場**で、既存構造を見ても分からない規約（あれば。無ければ「なし」）
4. ブランチ戦略（既定で問題なければ省略可）
5. 作成するドキュメント（上記の推奨から選択、または「なし」。**プロジェクト固有のドキュメントを追加指定**してもよい）
   - 雛形を作る場合は**ファイルを作成**します。作らない場合は `CLAUDE.md` には記録しません（未実在のパスを書くと、そのパスが毎回ロードされ続けるため）
