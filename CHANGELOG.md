...
- **同梱サブコマンド `claude-harness-run doctor` を `claude-harness-run preflight` へ改名した（Issue #244）。** Claude Code 本体のセッション内コマンド `/doctor` と同名で役割が違い、取り違えやすかったため。**旧名 `claude-harness-run doctor` は動作しなくなる**（後方互換のエイリアスは置かない。ランチャーは `scripts/<名前>.sh` で解決するため、旧名では該当スクリプトが見つからずエラーになる）。
  - ファイルは `scripts/doctor.sh` → `scripts/preflight.sh`、`scripts/specs/doctor.md` → `scripts/specs/preflight.md`、`scripts/tests/test-doctor.sh` → `scripts/tests/test-preflight.sh` に移った。診断の内容・引数・stdout の JSON・終了コードは変わらない。stderr の要約行の接頭辞は `doctor:` から `preflight:` になる。
  - 本体の `/doctor` は変わらない（本プラグインとは無関係）。
...
- **`/init-project` が生成する `CLAUDE.md` を棚卸しした（Issue #237）。** 公式の memory ガイド（1 ファイル 200 行未満を目標／コードベースから導ける内容は削り、pitfalls・rationale・ツール既定と異なる規約を残す）に合わせ、**次の 4 節を生成しなくなった**: `## プロジェクト概要`（architecture overview）・`## 技術スタック`（dependency list）・`## ドキュメントマップ`（directory layout。かつ「作成予定」の行が未実在パスを常時ロードさせていた）・`## 開発原則`（YAGNI/KISS/DRY。ツール既定と異なる規約ではない一般論で、検証できない曖昧さの典型）。`### 命名規則` の表も落とし、既存構造から導けない `### 新規ファイルの置き場` だけを残した。
  ...
  - 生成物の末尾に、セッション内 `/doctor`（Claude Code v2.1.206 以降）で棚卸しできる旨の案内が 1 行入る。**同梱の `claude-harness-run preflight` とは別物**（あちらは settings の allow/deny と生成物の追従を見るもので、行数は見ない）。
- **`/init-project` の生成物へ harness／プラグイン固有の語を書かない規定を追加した（Issue #239）。** 対象はスキル名（`/para-impl` 等）・ランチャー名（`claude-harness-run`）・プラグイン配下パス・harness のフロー名。`skills/init-project/SKILL.md` ステップ4 に明記し、混入を `claude-harness-run preflight` の新しい advisory チェック **`claude_md_harness_terms`** が検出する（検出語の正本は `skills/init-project/scripts/harness-terms.json` の固定語と、`skills/` 配下から実行時に導出するスキル名）。ステップ7 の完了報告は**会話に出す案内であって生成物ではない**ため対象外である旨も線引きとして書いた。
...
- **`claude-harness-run doctor` を呼んでいる独自のドキュメント・プロンプト・スクリプトがあれば `claude-harness-run preflight` へ書き換える**（フォールバックで `bash "<プラグインルート>/scripts/doctor.sh"` を直接呼んでいる場合は `scripts/preflight.sh` へ）。同梱のスキル・ドキュメント・テストは追随済み。permission の allow は `Bash(claude-harness-run:*)` のままで新名にも効くため、変更は要らない。
- **既存のプロジェクトの `CLAUDE.md` は何もしなくてよい。** 生成物はテンプレート追従を持たないため、既に生成済みの `CLAUDE.md` は変わらない。`claude-harness-run preflight` の `claude_md_sections` は**テンプレートから実行時に節を抽出する**ため、節が減ったことで新しい指摘が出ることもない（`claude_md_doc_map` は旧世代の生成物のために残してある）。
...
- **`/impl` を新設し、1チケットの実装フローの正本をここへ移した（Issue #233）。** `/para-impl` からは実装フローの手順そのものが無くなり、**fan-out（並列の起動）に徹する**形になった。`/impl` は通常のスキル（サブエージェントでは走らない）で、`/impl {番号} --base {base} --worktree {path}` の形で呼ぶ。
  - **経路の分岐は `--worktree` の有無ただ1つで決まる。** 在れば worktree の準備を呼び出し元が済ませたものとして該当 Phase をスキップし、E2E は `/create-e2e` までで切る（`/explain-e2e` は対話前提のためリードがメインセッションで実施する）。無ければ両方を `/impl` 自身が実施する。判定材料を2つ持たないため、経路の記述がずれない。
  - **並列度・直列化の決定権を上位層へ寄せた。** `/para-impl` は `--max-parallel` / `--serial` を受け取り、**そのとおりに**起動して判断し直さない。`issue-conflict-predictor` の予測は決定権を持たず、**`--max-parallel` を下回る方向にのみ**内側で追加直列化してよい（その事実と理由は 0 件でも完了報告に明記する）。**`--max-parallel` を渡さない従来の呼び方は後方互換**で、`/para-impl` 自身が並列度を決める。
  - 背景は ADR 0001 決定4「賢い調整層は常に1つ」。調整層が上位層と `/para-impl` の2つあり、全体の並列度・予算を見ている主体が居ない状態だった。

### 変更

- **`/para-impl` / `/impl` の Phase 番号を 1〜10 の通し番号にし、`feature-implementer` の内側を `Step a`〜`Step e` にした（Issue #235）。** 外側（`para-impl` / `impl`）と内側（`feature-implementer`）が独立した Phase 番号を持ち、同じ `Phase N` が層によって別工程を指していた。あわせて外側にあった `Phase 4-5` という範囲表記を解消し（`N-M` は階層専用の記法に統一）、欠番も無くした。内側の小節は `Step a-1`〜`Step e-3`。
  - **フロー図に必須ゲートの反復を描いた。** 凡例（`↓` 次へ進む / `↺` 同じ Phase 内の反復（上限あり） / `✗` 反復では解消しない失敗）を導入し、内側の `/quality-check` 最大3回リトライと、3回で解消しない場合の `failure` → チケットスキップを**別の事象として**読める形にした。従来の図は反復を落としており「一発で落ちたら即スキップ」に読めた。
  - `docs/adr/0004-self-review-convergence-by-severity.md` は確定済み ADR のため**本文を書き換えず**、新旧対応の注記を追記した（旧番号の記述が意図的な例外として残る）。

### 利用者が取る操作

- 単一 Issue の実装は `/impl {番号}` を直接呼べる。**`/para-impl` 経由の従来の呼び方は引き続き動く**ため、既存の使い方を変える必要はない。
- **Phase 番号で手順を指している独自のドキュメント・プロンプト・スクリプトがあれば、新しい通し番号（外側 1〜10）と `Step a`〜`e`（内側）へ追随させる。** 同梱のテスト・ドキュメント側は追随済み。
...
- **機能仕様を「スライス（出荷の単位）」に切り、既定を最小スライス S1 にした（Issue #228）。** `/define-feature` は要件ヒアリングの最後に「それだけで出荷でき価値が出る最小の縦切り（S1）」を必ず問い、テンプレート `feature-spec.md` に必須節 `## スライス（出荷の単位）`（表＋ `実装対象: {スライス ID}` 行。初期値は S1 で、後続スライスへ進めるときに書き換える）と `## やらないこと`（範囲外の明示列挙）が加わった。受入基準は実装対象スライスの範囲に収め、`spec-critic` の `internal-consistency` レンズがこれを照合する。`/create-ticket` 要件モードは実装対象スライスの要件チケット **1件だけ**を起票し、タイトル末尾に `（S1）` を付ける。後続スライスは**前スライスの PR がマージされた後**に、人間または呼び出し側フローが機能仕様の `実装対象:` を進めて再実行する（`/pr-merge` の後処理には組み込まない）。スライス表の無い既存の機能仕様は従来どおり 1 チケットとして起票される（後方互換）。スライス（出荷単位）と粒度（スライス内の分割）の関係は [`skills/create-ticket/references/slice-vs-granularity.md`](skills/create-ticket/references/slice-vs-granularity.md)。
