...
- 起票失敗件（`status: "failed"`）: 失敗理由（`error`）とともに報告する。失敗結果には元項目の内容ではなく `index` のみが含まれるため、**保持しておいた元 manifest から該当 index の項目を抽出して新しい manifest を作り、`${CLAUDE_PLUGIN_ROOT}/scripts/create-debt-issues.sh` を再実行すれば再起票できる**旨を伝える。再起票が完了し対応表がすべて `created` になったら、一時ファイルは削除してよい
