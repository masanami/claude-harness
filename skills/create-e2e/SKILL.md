...
- Issue/PRの場合: `gh issue view` / `gh pr view` で本文・完了条件・受入基準を取得する。加えて `scripts/extract-acceptance-criteria.sh <番号>` で完了条件を機械可読JSON（`{issue, criteria:[{id,text,checked}], parse_status}`）としても取得しておく（後続 Step 1-3 のトレーサビリティチェックの入力になる）
