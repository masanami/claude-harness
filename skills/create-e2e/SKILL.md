...
- 実装は `e2e-engineer` エージェントに委譲してもよい（Phase 1〜3 までを担当）
...
実装と実行が完了したら、レビュー用に **`/explain-e2e`** を実行する。
このスキルは「テストシナリオ解説」（戦略 §3.3 の中間層）と「独立検証」（同最下層）を**メインセッションで対話的に**実施する。サブエージェント（`e2e-engineer`）に委譲した場合は、メインに戻ってから `/explain-e2e` を実行すること。
