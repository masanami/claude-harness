...
- **`/impl` の呼び出し形**（実装フローの手順そのものは**注入しない**——正本は `/impl` ただ1つであり、プロンプトへ再掲すると正本が2つになって必ずずれる）:
  ...
  `--worktree` を渡すことで `/impl` は Phase 3（リードが worktree 作成で実施済み）をスキップし、Phase 7 を `/create-e2e` までに切る。**`/impl` は通常のスキルであり、その呼び出しは Task ネスト深度を消費しない**（worker〔深度1〕→ `feature-implementer`〔深度2〕→ `code-reviewer`〔深度3〕の鎖は現状のまま保たれる）
