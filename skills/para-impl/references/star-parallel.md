...
Phase 1〜2 の結果をもとに実行計画を提示する。これは**報告であり承認ゲートではない**（実装フェーズに人間ゲートは無い）。

**`--max-parallel` を受け取っている場合、この表は「上位層から受け取った計画の写し」であって、リードが立て直した計画ではない**（SKILL.md「Phase 2」の規律1〜3）。`--max-parallel` が無い単独利用の場合のみ、リードが自分で並列度・直列化を決めてこの表を作る。その場合で Issue 間の依存関係・衝突の判断が必要かつリード自身が headless で確認できないときは、**保守側（直列化）に倒し**、完了報告に判断事項として明記する:
...
同時起動の天井: {--max-parallel の値 | 単独利用のためリードが決定: N}
直列化グループ: {--serial の値 | 単独利用のためリードが決定 | なし}
追加直列化: {予測交差を根拠に内側で追加した組と理由 | なし}
```
...
>
> **予測は直列化の決定権を持たない。** `--max-parallel` を受け取っている場合、予測が使えるのは **`--max-parallel` を下回る方向への追加直列化だけ**であり、受け取った `--serial` グループの解除・天井の引き上げには使えない（SKILL.md「Phase 2」の規律4）。**予測のために上位層へ戻らない**（同一周のラウンドトリップ禁止）。追加直列化した場合は、その組と根拠にした予測交差を完了報告に必ず書く。

### 直列化の判断（残留結合の吸収）

**`--max-parallel` を受け取っている場合、直列化の決定権は上位層にある**。リードは受け取った `--serial` グループをそのまま守り（解除・組み替えをしない）、同時起動数を `--max-parallel` 以下に保つ。予測で見つけた追加の衝突は**天井を下回る方向にのみ**内側で吸収し、その事実と理由を完了報告に書く。**以下の判断（どの組を直列化するか）をリードが行うのは、`--max-parallel` を受け取っていない単独利用のときだけ**である。

「独立したチケット」でも残留結合（共有ファイル・共通型・PR マージ順）はゼロにならない。次で吸収する:
...
- **`/impl` の呼び出し形**（実装フローの手順そのものは**注入しない**——正本は `/impl` ただ1つであり、プロンプトへ再掲すると正本が2つになって必ずずれる）:
...
| worker の返却 | リードの動作 |
|---|---|
| 通常完了（PR URL・CI `green` / `none`。`none` は CI チェック未設定リポジトリの green 相当） | Phase 9 の集約へ。E2E対象チケットは worker 返却のシナリオ一覧を使い、リードが `/explain-e2e` をメインセッションで実施 |
...
| 返却未受領（未合流のまま） | **この表のどの行にも該当しない＝処理完了ではない**。Phase 9 へ進まず、「合流ゲート（最終応答前の未合流確認）」（`references/join-gate.md`）の決定表に従い、ツール呼び出しで合流を続けるか、断念して中断報告へ倒す |
...
4-b. **同時起動の天井・直列化グループの出所（`--max-parallel` / `--serial` を受け取ったか、単独利用でリードが決めたか）と、内側で追加直列化した組・その理由**（追加が0件なら「追加直列化なし」と明記する。上位層が次周の実行計画の入力に使う）
...
7. **worktreeの状態**: 各 worker の worktree が保持されていることを報告し、レビュー対応後に Phase 10 でクリーンアップする旨を伝える
...
> **重要**: 複数Issueの場合、この時点でworktreeを削除しません。PRレビューで指摘が見つかった場合、修正のためにworktreeが必要です。クリーンアップは Phase 10 で行います。

---

## Phase 10: Worktreeクリーンアップ（複数Issueの場合）
...
|---------|-------------|--------------|------------|
| 単一Issue | 不使用 | - | - |
| 複数Issue | リードが `scripts/worktree-setup.sh` で作成し ticket-worker が使用 | 全PRのレビュー対応完了後、またはユーザーの明示的な指示 | Phase 10（`scripts/worktree-cleanup.sh`） |
