...
6. **次のアクションの案内**: レビューコメントが付いたPRだけ `/pr-review-respond`。マージは base=統合ブランチならリードが `/pr-merge` で自律実行、base=既定ブランチならユーザーに `/pr-merge` を案内（項目8参照）
...
> 2. PR作成済み
> 3. 当該PRに未対応レビューコメントがない（コメントがある場合は `/pr-review-respond` による対応が完了している）
