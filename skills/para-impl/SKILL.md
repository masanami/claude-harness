---
name: para-impl
description: "GitHub Issueを分析し、設計→クリティカル判定→TDD実装(エージェント内でQC通過まで)→コミット→E2E→PR→CI確認の1チケットフローを実行する。複数Issue時はAgent Teams構成を提案する。Triggers on: '/para-impl', '並列実装', 'Issueを実装して'"
...
GitHub Issueを分析し、AI駆動開発戦略の1チケット実行フロー（設計→クリティカル判定→TDD実装→必須ゲート→コミット→E2E→PR→CI確認）に沿って実装を進めます。Issueが複数の場合はAgent Teamsの構成をユーザーに提案します。

実装エージェント（`feature-implementer`）はサブエージェントから `/quality-check` を Skill 動的呼び出しで実行できるため、TDD実装と必須ゲート通過を**ひとまとまり**として委譲します。
...
単一Issueはリードエージェントが、複数Issueは各teammateがworktreeで以下を実行する（**1チケット = 1ブランチ = 1PR**）。設計→クリティカル判定→TDD実装（必須ゲート＋軽量セルフレビュー内包）→コミット→E2E→PR→CIの順で進める。
...
Phase 5 TDD実装 + 必須ゲート + 軽量セルフレビュー（エージェント内で完結）
   ↓（failure → 当該チケットをスキップ）
...
`feature-implementer` エージェントを **`design-only` モード** で呼び出す。例:

> 「Issue #{番号} について `design-only` モードで Phase 1〜2 を実施し、Phase 3 の実装には進まず停止してください。
> 成果物: 設計方針・影響範囲・実装計画、E2E対象であればE2Eシナリオと完了条件トレーサビリティ表、クリティカル箇所があればその申告。」
...
- エージェントが**クリティカル箇所を申告**した場合 → 実装計画と影響範囲を提示
  - クリティカル箇所の例: セキュリティ（認証・認可・入力検証）、機密データ・個人情報、外部システム連携、DBスキーマ変更
  - エージェント側で申告漏れが疑われる場合はリードが上記基準で補強判定する
- E2E対象機能 → E2Eシナリオ・トレーサビリティ表を提示し、カバレッジの過不足を確認

両方該当する場合は同じターンで両方提示する。**どちらにも該当しない**場合は AI完結で即 Phase 5 へ進む。

### Phase 5: TDD実装＋必須ゲート＋セルフレビュー

Phase 4 で確定した設計成果物を渡し、`feature-implementer` エージェントを **`implement` モード** で呼び出す。エージェントは以下まで自身で責任を持つ:

1. TDD（Red → Green → Refactor）の実装
2. **`/quality-check` を Skill 動的呼び出し**で必須ゲート通過（Lint / 型チェック / テスト）
3. **軽量セルフレビュー**（完了条件達成・スコープ確認・テスト設計妥当性）
...
エージェントが `failure` で戻した場合:

- リードはチケットを **スキップ扱い** とし、後続フェーズ（Phase 6 以降）に進まない
- 並列モードの場合は当該 teammate だけ停止し、他は継続
...
`/commit` は **コミット規約に従ったコミット実行に責務を絞った**スキル。内部では safety net として `/quality-check` を再走させ、Conventional Commits 形式でコミットを作成する。Phase 5 で必須ゲート・セルフレビューを通過済みのため、ここでの `/quality-check` は通過前提で速やかに完了する。
