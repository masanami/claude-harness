...
- **合流済み**: 有限タスクは**最終返却（結果）をこのセッションで受領した**状態。常駐サービスは**停止・後始末を実施し、停止を確認して台帳に記録した**状態（突合済み＝合流相当）。「起動に成功した」「完了通知が来るはず」「稼働中である」は合流ではない。
...
起動台帳が把握できるのは**自分が直接起動したものだけ**である。委譲先（`ticket-worker`・`feature-implementer` 等）が自身でさらにサブエージェントや常駐サービスを起動する場合、そのネストの解消は台帳では検査できない。そのため、サブエージェントへ委譲する spawn プロンプトには、次の**合流ゲート伝播条項**を逐語で含める（Task ツールを持たないエージェントへの委譲でも含めてよい・害はない。含めるか迷った場合は含める）。**ネストの解消は各委譲先が本条項で保証する**——委譲先からの**条項 (3) の未解消報告を含まない正常返却**の受領は、委譲先がネストを解消済みであることを含意する。**未解消報告を含む返却は合流済みとして扱わず、台帳に「ネスト未解消」として記録する**（決定表のネスト未解消行が親の中断報告へ倒し、委譲先が報告した未解消の一覧・実状態を転記する）:
