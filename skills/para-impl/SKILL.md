...
- **起動台帳**: このセッションで自分が起動したサブエージェント（Task ツール等）・バックグラウンド処理の一覧（種別・担当Issue・worktree パス／作業ブランチ）。起動のたびに追記して保持する。
- **合流済み**: 当該エージェント・処理の**最終返却（結果）をこのセッションで受領した**状態。「起動に成功した」「完了通知が来るはず」は合流ではない。
- **未合流**: 起動台帳に載っており、合流済みでないもの。

### 手順（spawn 時）

1. サブエージェント・バックグラウンド処理を起動したら、その場で起動台帳に追記する。
2. **起動直後にテキストで応答を確定しない**。「完了を待ちます」「通知が来次第続行します」等の待機宣言をテキスト応答として出力した時点で、headless ではセッションが終了し起動済みエージェントは全滅する。待機宣言は注意書きではなく**手順違反**として扱う。完了通知・返却が届くまで**ツール呼び出しを続けてターンを維持する**（結果取得ツールでのブロッキング待機、または状態確認のポーリングを合流まで繰り返す）。

### 手順（最終応答の直前）

起動台帳と合流済みの記録を突き合わせ、次の決定表で分岐する。**各行の状態は互いに排他であり、どの状態も高々1行にだけ該当する**（1〜4行目は台帳と実状態の突き合わせが成立していることが前提。成立しない場合に該当するのは最終行だけ）。**どの行に該当するか判定できない場合は、突き合わせ不能として最終行（中断報告）へ倒す**。受領の見込みは**実状態の確認で判定し、待機・ポーリングの回数や経過時間だけを根拠に判定しない**（結果取得のタイムアウトは「結果がもう来ない」ことを意味しない。Phase 4〜9 を実行中の worker は長時間・多数回の待機にまたがって正当に稼働し続ける）。再試行上限（**3回を目安**）は、**稼働確認も結果取得もできない場合の確認試行にだけ**適用する:
