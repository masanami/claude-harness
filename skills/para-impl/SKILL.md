...
起動台帳と合流済みの記録を突き合わせ、次の決定表で分岐する。**各行の状態は互いに排他であり、どの状態も高々1行にだけ該当する**（1〜4行目は台帳と実状態の突き合わせが成立していることが前提。成立しない場合に該当するのは最終行だけ）。**どの行に該当するか判定できない場合は、突き合わせ不能として最終行（中断報告）へ倒す**。合流の再試行上限は**3回を目安**とし、結果取得の失敗も回数に数える:
