...
3. どちらも無ければ **base = `main`**（従来動作）

base が `main` 以外（統合ブランチ）の場合、そのブランチが remote に存在することを前提とする（無ければユーザーに作成を促す）。統合ブランチへのサブタスク PR マージは本番影響がなく可逆のため**人間承認不要で自律マージできる**（詳細は [ブランチ戦略 2.5](../../docs/branching-strategy.md) / [承認ゲート 3.5](../../docs/ai-driven-development-strategy.md)）。以降のフローで **`{base}` は上記で決定した base ブランチ**（既定 `main`）を指す。
...
`{base}`（「base 統合ブランチの決定」で確定した base。既定 `main`）から作業ブランチを切る:
...
**PR の base は Phase の冒頭で決定した `{base}`**（既定 `main`、統合ブランチ方式では統合ブランチ）にする:
...
> `base` は各 Issue の PR 宛先（既定 `main`）。`--base` 指定時や Issue 本文の `Base:` 行がある場合は統合ブランチを表示する。
...
7. **統合ブランチ方式の場合**: 全サブタスク PR を統合ブランチへ自律マージ後、統合ブランチでの最終動作確認と `main` 昇格が残る唯一の人間ゲートである旨を案内する（手順は [ブランチ戦略 2.5.3 統合 → main 昇格手順](../../docs/branching-strategy.md)）
