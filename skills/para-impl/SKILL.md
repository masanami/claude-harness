...
- **クロスリポジトリ依存の確証結果**（該当する場合。仮定と確証根拠 repo@ref/file:line（コミット SHA 等でリビジョン固定）、未確証の仮定の明示。Phase 8 で PR 本文に転記する）
...
実装が**クロスリポジトリ依存の仮定**（他リポジトリの API の副作用・導出・判定ロジックへの依存）を含む場合、Phase 4-5 で受け取った**確証結果（仮定・根拠 repo@ref/file:line・未確証の仮定）を PR 本文に記載する**。レビュアーが仮定の真偽を根拠付きで検証できるようにするため。**自リポジトリのモックテスト green は仮定の検証根拠にならない**。
