...
## サブエージェント・teammate への共通指示（プロンプトに必ず含める）
...
リードは要件チケット本文の **「クリティカル設計決定」セクション**をエージェントに渡し、その方針に従って実装するよう指示する。あわせて**「サブエージェント・teammate への共通指示」（permission 拒否時の振る舞い・headless 制約）を必ずプロンプトに含める**。
...
各 teammate への指示には**「サブエージェント・teammate への共通指示」（permission 拒否時の振る舞い・headless 制約）を必ず含める**。teammate がさらにサブエージェント（`feature-implementer` 等）を呼ぶ場合も、同じ指示を伝播させるよう teammate に指示する。
