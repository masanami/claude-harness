...
単一Issueはリードエージェントが、複数Issueは各 `ticket-worker` が worktree で以下を実行する（**1チケット = 1ブランチ = 1PR**）。設計→TDD実装（必須ゲート＋セルフレビュー内包）→コミット→E2E→PR→CIの順で進める。**実装フェーズに人間ゲートは無い**。
...
> **複数Issue（star 型）の場合**: worker は `/create-e2e` までを実施し、`/explain-e2e` は対話前提のため worker 完了後に**リードがメインセッションで実施**する。
...
Phase 1-2 の分析結果をもとに実行計画を提示する。これは**報告であり承認ゲートではない**（実装フェーズに人間ゲートは無い）。Issue 間の依存関係・衝突で判断が必要な場合のみユーザーに確認する:
...
- Issue 間に依存関係がある、または同一ファイル群への変更が Phase 1 で見えている場合は**並列にせず直列化**する（先行チケットの PR マージ後に後続 worker を spawn）
- それでも統合時に衝突した場合は**リードがコンフリクトを解決**する

### worker への委譲

**事前確認（permission 拒否の予防）**: gitignore された `.claude/settings.local.json` は worktree にコピーされないため、allow 権限は **git tracked の `.claude/settings.json`** に無ければ worktree 内の worker に適用されない（`/init-project` のステップ 4b 参照）。worker を spawn する前に必要な権限が揃っているかを確認し、不足があればユーザーに案内する。
...
| `failure` | 当該チケットをスキップして報告。他 worker は継続 |
...
6. **次のアクションの案内**: 各PRに対し `/pr-review-respond`、マージ可能になり次第 `/pr-merge`
...
```bash
git worktree list
git worktree remove {worker-1のworktreeパス} --force
git worktree remove {worker-2のworktreeパス} --force
git worktree list
```
