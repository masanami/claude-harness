...
- **未合流**: 起動台帳に載っており、合流済みでないもの。区別が記録されていない・判別できない起動が残っている場合は、決定表の「どの行に該当するか判定できない場合」の既定（中断報告）に従う。
...
起動台帳が把握できるのは**自分が直接起動したものだけ**である。委譲先（`ticket-worker`・`feature-implementer` 等）が自身でさらにサブエージェントや常駐サービスを起動する場合、そのネストの解消は台帳では検査できない。そのため、サブエージェントへ委譲する spawn プロンプトには、次の**合流ゲート伝播条項**を逐語で含める（Task ツールを持たないエージェントへの委譲でも含めてよい・害はない。含めるか迷った場合は含める）。**ネストの解消は各委譲先が本条項で保証する**——委譲先からの返却の受領は、委譲先が本条項を守った（ネストを解消済みである）ことを含意する:
