...
- **`/quality-check` の最終結果**（`pass` / `skip`（ゲート未実行＝未検証） / `failure`）
...
| `skip`（`/quality-check` の品質ゲートが1つも実行されていない＝未検証） | Phase 6 へ進んでよいが、**`pass` として扱わず**、未検証である事実と対象チケットを PR 本文・完了報告に明記する（品質ゲートを通過したかのように報告しない） |
