...
該当するコマンドが存在しないものは省略する（手順2のスクリプトが「スキップ」として扱い、失敗とはしない）。**ただし lint / 型チェック / テストのいずれも特定できなかった場合**は、手順2は「実行すべきゲートが1つも無い＝何も検査していない」として `result: "skip"`（exit 3）を返す。これを `pass` に読み替えず、**未検証として報告する**（手順2の解釈・手順3）。
...
2. **stdout が妥当な JSON** → 品質結果として解釈する。exit code は `result` が `pass` なら 0、`fail` なら 1、`skip` なら 3
   ...
   - **`result: "skip"`（exit 3）は「実行すべきゲートが1つも無く、何も検査していない」状態**（`--lint`/`--typecheck`/`--test` をすべて省略した実行）。品質 fail ではないが、**`pass` にも読み替えない**（検査していないものを「全ゲート通過」として報告しない）。手順1でコマンドを取りこぼしていたなら特定し直して再実行し、プロジェクトに本当に lint / 型チェック / テストが存在しない場合は**未検証（⊘ SKIP）として手順3で報告する**（呼び出し元が「品質ゲートを通した」と誤読しないよう、総合判定を `PASS` にしない）
...
手順2で得た stdout の JSON（`{result, auto_fix, gates}`）を、**機械可読な結果として最後にそのまま出力**する（呼び出し元のスキル/エージェントが判定に使う）。`result` が `"skip"`（ゲート未実行）の場合も**そのまま `"skip"` を出力し、`"pass"` に書き換えない**。
...
### 総合判定: ✅ PASS / ❌ FAIL / ⊘ SKIP（ゲート未実行＝未検証）
```
