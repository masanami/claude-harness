...
**親Issue本文に台帳の読み取り手段（索引チェック。`### G-...` の見出しを保証見出しとみなす読み方）を向けないこと**: 親Issueの保証節は `### 新たに宣言する保証` のような**カテゴリ見出し**を持ち、保証 ID は見出し行ではなく配下のリスト行にあるため、**親Issueへ適用してよいのはフェンス・節の範囲・チェックリスト行の扱いだけ**である（親Issue本文の文法は 5.5-3 が定義する）。台帳の文法で親Issueを読むと、正しく作られた親Issueを `guarantee_section_missing` と誤判定する。
...
**本手順に適用されるのは、この定型文のうち「索引チェックへ渡す引数の解決」だけ**である。**本ファイルの手順は台帳を Read しない**（読み取りは 5.5-4 のスクリプトへ一本化済み。上記「台帳の読み取りの正本」）。定型文が「台帳の Read にも」と書いているのは、台帳へ**書き込む**手順（`feature-implementer` の台帳追記・`/guarantee-audit` bootstrap のドラフト生成）と共通の文面だからであり、**本手順で台帳を Read する根拠にはならない**。
