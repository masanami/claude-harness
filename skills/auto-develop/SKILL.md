...
E2Eテスト対象機能（認証・権限・クリティカルパス等）の場合、create-e2e スキルの手順に従い、仕様（完了条件・受入基準）からトレーサビリティ・解説生成を含めてE2Eを作成する（非対話。全自律のためユーザー動作確認は行わない）。
...
- **E2E**: 対象機能は create-e2e の手順で作成（非対話・仕様ベース。全自律のため動作確認は行わない）
