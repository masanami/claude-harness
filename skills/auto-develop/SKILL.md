...
E2Eテスト対象機能（認証・権限・クリティカルパス等）の場合、`/create-e2e` の手順に従い、仕様（完了条件・受入基準）からトレーサビリティ表→実装→実行までを行う。続いて `/explain-e2e` の手順でテストシナリオ解説と独立検証を実施する（全自律モードのためユーザー対話・動作確認は行わず、解説生成と独立検証のみAI主導で完了させる）。
...
- **E2E**: 対象機能は `/create-e2e` で実装→実行、続けて `/explain-e2e` で解説生成と独立検証（全自律のためユーザー対話・動作確認は省略）
