...
> **スクリプトの所在（重要）**: 本スキルはプラグインとして配布されるため、スクリプトは**ユーザーのプロジェクトroot ではなく、プラグイン配下**にある。実行時にエージェントへ渡される**このスキルの base directory**（= `${CLAUDE_PLUGIN_ROOT}/skills/walkthrough`）配下の `scripts/` を参照すること。以下では `<skill_dir>` と表記する。`cd` はせず、**dev server を起動したプロジェクトを cwd にしたまま**スクリプトを実行する（runner は cwd の git root から `@playwright/test` を解決する）。
...
2. **セットアップスクリプトを実行**
   - `bash <skill_dir>/scripts/walkthrough-setup.sh` を実行する。
...
4. **runner で起動**
   - `node <skill_dir>/scripts/run-walkthrough.mjs [flow.mjs]` を**プロジェクトを cwd にしたまま**実行する。
