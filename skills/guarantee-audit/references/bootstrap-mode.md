...
`indeterminate` / `unreadable` / 応答なしが1件でもあれば、**この bootstrap の抽出は網羅的ではない**。件数（保証・GAP・要判定・内部除外）を「検査した結果」として提示せず、未解析が残っている旨を必ず併記する。
...
（**解析できなかったファイルが1件でもあれば、上の件数は「検査した結果」ではなく下限である**。その分の公開面は台帳ドラフトにも Gaps にも現れていない）
