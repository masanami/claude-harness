...
<!-- 未解析あり（該当する場合のみ記載）: 未解析の公開面カテゴリ {n} 件 / 抽出に失敗したテストファイル {n} 件。
     このドラフトの Gaps は網羅的ではありません。詳細は bootstrap 実行時の報告を参照してください。 -->
...
- **Step B3 に抽出失敗（`extraction_failed` / `unreadable`）があった場合、または Step B4 に `not_analyzed` のカテゴリがあった場合は、冒頭の「未解析あり」コメントに件数を記載する**（記載が不要な場合はコメントごと削除する）。ドラフトだけを見た人間が、網羅的な台帳だと誤解しないようにするため（報告と成果物が別々に流通しても、成果物側だけで未解析の存在が分かる状態を保つ）。
