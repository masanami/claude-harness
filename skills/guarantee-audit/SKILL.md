...
Step 1 の完了後、モードに応じて**下表の「参照ファイル」列のパスを前掲の配送経路で読み出し**、その手順に従う（`claude-harness-run read-plugin-doc "<下表のパス>"`。**選んだモードの行のパスを使うこと**。Read 直読みは前掲の注記のとおりランチャー未導入時のフォールバックに限る）:

| モード | 参照ファイル（配送経路へ渡すパス） | 手順 |
|---|---|---|
| bootstrap | `skills/guarantee-audit/references/bootstrap-mode.md` | Step B1〜B6 |
| drift | `skills/guarantee-audit/references/drift-mode.md` | Step D1〜D5 |
