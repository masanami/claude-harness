...
<!-- /guarantee-audit bootstrap が生成したドラフト。人間の裁可 PR で ID と宣言元を確定し、
     docs/guarantees.md へ正本化してください。このドラフトのままでは索引チェックは通りません
     （ID が仮置きのため）。 -->

## 保証（Guarantees）

### G-?-1: {behavior_ja をそのまま約束文にしたもの}
...
- [ ] GAP-?-1: {列挙した公開面の要素}（テスト未整備）
```

書式の規約:

- **ID は仮置き**とし、`G-?-{枝番}` の形で枝番のみ通し番号を振る（`?` の部分は裁可 PR の番号で確定する）。GAP も同様に `GAP-?-{枝番}` とする。**このドラフト内で正式な番号を採番しない**（採番できる場所は宣言元 Issue の保証節と台帳更新 PR だけであり、監査は候補を報告するだけ、という単一経路を守るため）。
- `宣言元` は全件 `裁可待ち` と記載する。
...
**このドラフトは意図的に `guarantee-index-check` を通らない**（仮 ID が `G-{数字}-{枝番}` 書式に合致しないため）。正本化時に ID を確定して初めて索引チェックの対象になる。
...
2. 裁可 PR を作り、仮 ID（`G-?-n` / `GAP-?-n`）をその PR 番号で確定する（`G-{PR番号}-{枝番}` / `GAP-{連番}`）。`宣言元` も PR 番号に置き換える
