...
5. 開発フェーズ
   ...
   - 宣言なし・保証台帳（`docs/guarantees.md`）あり → **SDD期**（既定）。GDD期 は候補として提示しません（GDD レジームは不採用・凍結。既存の台帳は凍結した資産として残っているだけであり、新規の宣言は起こしません）
   - 判定が `invalid`（宣言の書き間違い）・判定スクリプトの実行失敗 → **要人間判定**。既定値は選ばず、フェーズ節を生成しません。上記の判定理由・判定元を見て `CLAUDE.md` の宣言を修正してください
     - 判定スクリプト自体が実行できなかった場合（判定理由・判定元が得られない場合）は、上の2欄に**スクリプトの stderr メッセージ・判定を試みた対象・修正手順**を記載します
   - 既に有効な宣言がある場合、本スキルはフェーズ値を書き換えません。切り替えを希望する場合は、駆動文書の移行手順（保証台帳の用意 → 裁可 → 機能仕様の退役 → ドキュメントマップ更新）とあわせて別途実施してください
