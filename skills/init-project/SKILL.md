...
| 宣言なしかつ `docs.guaranteesLedger` が非 null（保証台帳が既にある） | **`SDD期`**（既定）。**`GDD期` を候補として提示しない**——GDD レジームは不採用・凍結であり、新規に宣言を起こす提案をしないため。台帳の存在は「凍結した GDD の資産が残っている」事実として検出結果に載せるにとどめ、フェーズの推奨値には使わない |
...
- **本スキルが `GDD期` を書き込めるのは、既存宣言が `declared_gdd` の場合だけ**である（維持のみ）。ユーザーから `GDD期` を要望された場合も、本スキルはフェーズ値を新規に `GDD期` へ確定しない——GDD は不採用・凍結であり、宣言を新たに起こす経路を初期設定スキルに残さないため。要望を受けたら凍結の事実を案内し、この実行では `SDD期` とする
<!-- 凍結の決定と根拠: docs/adr/0002-gdd-not-adopted-salvage-instruments.md ／ 既定フロー: docs/ai-driven-development-strategy.md 4.4 -->
- 現状維持（`SDD期`）には確認ゲートを増やさない（提示に含めて訂正できる形にすれば足りる）
...
- **開発フェーズ**: `{DEV_PHASE}` に Step 3 で確定した値（`SDD期` または `GDD期`）を、`{DRIVING_DOC}` に対応する駆動文書のパス（SDD期: `docs/features/`、GDD期: `docs/guarantees.md`。CLAUDE.md 側で別パスを採用する場合はそれ）を入れる
  ...
  - `GDD期` を書き込んでよいのは、既存宣言が `declared_gdd` の場合のみ（Step 3 のとおり、新規に `GDD期` を確定する経路は持たない）
