---
name: init-project
description: "プロジェクトを分析してCLAUDE.mdと.claude/settings.jsonを自動生成する。整備すべきドキュメントの選定とブランチ戦略の決定も行う。Triggers on: '/init-project', 'プロジェクト初期設定', 'CLAUDE.mdを作成'"
---
...
- いずれの場合も、開発フローの**最小契約「1チケット = 1ブランチ → PR → 必須ゲート通過後にマージ」**を満たすこと（→ [AI駆動開発戦略](../../docs/ai-driven-development-strategy.md)）

#### 2h. 整備すべきドキュメントの候補抽出

必須は `CLAUDE.md` のみ。それ以外は検出した特性から**作成を推奨するドキュメント**を候補として挙げる（実際に作るかはステップ3で選定）:

| ドキュメント | 推奨条件 | 優先度 |
|------------|---------|--------|
| コーディングガイドライン | 常に推奨（コード規約の統一） | 高 |
| API仕様書 | Backend/API を検出 | 中 |
| テーブル定義書 | DB を検出 | 中 |
| アーキテクチャ設計書 | 複数サービス/大規模構成を検出 | 中 |
| 用語集 | ドメイン用語が多い業務系 | 低 |

> 既に存在が検出されたドキュメント（2e）は「整備済み」として扱い、候補からは「作成予定」として未整備のものだけを挙げる。
...
必須は CLAUDE.md のみです。特性から以下を推奨します。作成するものを選んでください（「なし」も可）:

| ドキュメント | 推奨理由 | 優先度 |
|------------|---------|--------|
| {ドキュメント名} | {推奨理由} | {高/中/低} |

> 2h の候補から、プロジェクトに該当するものだけを1ドキュメント1行で列挙する。
...
5. 作成するドキュメント（上記の推奨から選択、または「なし」）
```

> **ポイント**: 検出結果をそのまま提案し、ユーザーは修正したい箇所だけ指摘すればよい形にする。全項目の逐一確認は避ける。ブランチ戦略と推奨ドキュメントも既定/推奨を提示し、ユーザーは変更点だけ指摘すればよい。
