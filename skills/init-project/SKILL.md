...
> **スクリプトの所在（重要）**: 本スキルはプラグインとして配布されるため、スクリプトは**ユーザーのプロジェクトroot ではなく、プラグイン配下**にある。スクリプトは `${CLAUDE_PLUGIN_ROOT}/scripts/analyze-project.sh` を参照すること（`${CLAUDE_PLUGIN_ROOT}` は実行時にプラグインルートへ展開される）。分析対象ディレクトリ（引数）にはユーザープロジェクトの対象パスを渡す。
...
**出力**（stdout にJSON1個）: `{"status":"ok","target":"...","created":bool,"merged":bool,"allow_count":N,"deny_count":M}`。`status: "error"` の場合はエラー内容を確認する（jq不在・入力JSON不正など）。
