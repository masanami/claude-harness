...
- 手順1の「設計ドキュメントの探索」で特定した関連設計ドキュメントのパス（サブエージェントがコードベースと合わせて Glob/Grep/Read で調査できるように）。特定できていない場合は「未特定」と明示し、サブエージェント側に同じ探索手順（`docs/**/*.md` の Glob 列挙）を指示する
- 報告フォーマットの指示:
