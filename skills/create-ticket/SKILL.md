...
- 目的・全体像は [ブランチ戦略 2.5 統合ブランチ運用](../../docs/branching-strategy.md) と [AI駆動開発戦略 3.5 承認ゲート](../../docs/ai-driven-development-strategy.md) を参照。
- 未指定時は従来どおり base = `main` を前提とする（各チケットに base 記録は付けない）。
- 要件モードで指定された場合は無視し、その旨をユーザーに知らせる。
...
次のステップ:
# 統合ブランチが未作成なら先に作成（main から分岐）
git checkout -b {統合ブランチ} origin/main && git push -u origin {統合ブランチ}
