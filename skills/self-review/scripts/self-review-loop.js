...
//     エージェント（agentType: 'claude-harness:git-ops'。agents/git-ops.md）に委譲する
...
//     （agentType: 'claude-harness:feature-implementer'。Edit/Write を持つ。code-reviewer/design-reviewer/
...
// API 制約で最上位 type は 'object' でなければならない — 実機確認: 発見#4）。
...
function buildReviewPrompt(diffInfo, mode, previousFindings = []) {
  if (mode === 'confirmation') {
    return [
      ...
      '',
      '指定された JSON Schema（{findings: [{file, line, severity, claim, evidence, verdict}, ...]} の形のオブジェクト）に厳密に準拠したJSONのみを返してください。',
    ].join('\n');
  }
  return [
...
async function runReviewStage(diffInfo, mode, previousFindings, { agent, parallel, log }) {
  ...
  const [codeResult, designResult] = await parallel([
    () => agent(prompt, { agentType: 'claude-harness:code-reviewer', schema: FINDINGS_SCHEMA, phase: 'Review', label: `review:code:${mode}` }),
    () => agent(prompt, { agentType: 'claude-harness:design-reviewer', schema: FINDINGS_SCHEMA, phase: 'Review', label: `review:design:${mode}` }),
  ]);
  // agent() は terminal エラー時に null を返す。null を「指摘ゼロ」として集計すると
  // レビュー未実施のまま converged: true を報告する偽収束になる（実機確認: 発見#5）ため、
  // レビュアーのどちらかが落ちた場合はループを止めて明示的に失敗させる。
  if (codeResult === null || designResult === null) {
    const failed = [codeResult === null ? 'code-reviewer' : null, designResult === null ? 'design-reviewer' : null]
      .filter(Boolean).join(', ');
    throw new Error(`self-review-loop: reviewer agent(s) failed terminally: ${failed}. レビュー未実施のため収束判定を行わない。`);
  }
  const codeFindings = codeResult.findings;
...
const resolvedArgs = (() => {
  if (typeof args === 'string') {
    try {
      ...
      throw new Error(`self-review-loop: args is a string but not valid JSON: ${e.message}`);
    }
  }
  return args || {};
})();
