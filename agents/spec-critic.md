...
- **スコープの整合（「## スライス（出荷の単位）」と「## やらないこと」は必須節）**: 「## スライス（出荷の単位）」（スライス表と `実装対象: S1` の行）と「## やらないこと」の**いずれかが無い、または「やらないこと」の列挙が0件・「特になし」のみ**なら `needs_user_input`（範囲外と最小の縦切りはユーザーの意図でしか埋められない。`suggested_fix` には「S1 に入れないもの・範囲外を挙げてください」という確認文を書く）。両節が在るときは、**「## 受入基準」の各項目が `実装対象:` 行のスライス（既定 S1）の「内容」に収まっているか**を照合する。後続スライス（S2 以降）の内容や「やらないこと」に挙げた範囲外を検証する受入基準は blocker 候補（`suggested_fix` は当該項目を受入基準から外し、後続スライスの「内容」に留める案。ただし S1 の範囲そのものを広げるべきかが争点なら `needs_user_input`）
