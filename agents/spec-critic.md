---
name: spec-critic
description: "機能仕様ドキュメント（docs/features/{slug}.md）を3レンズ（受入基準の検証可能性/内部整合/下流実装可能性）のいずれかの観点で批評する際に使用。skills/define-feature/scripts/spec-critique.js（Dynamic Workflow）から `agentType: 'spec-critic'` として、focus値を変えて3体 parallel 起動される。"
tools: Read, Grep
model: sonnet
