...
#   (R-8) 規約が強制を委ねている先（生成側 `feature-implementer` Step c-1 と、検出側
...
assert_file_contains "(R-8) 正本が強制の所在として生成側を指している" "$CONV_FILE" \
  '`agents/feature-implementer.md` Step c-1'
