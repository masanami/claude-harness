...
#   (A-2) 同じファイルが、`CLAUDE.md` に無い項目（開発原則・技術スタック・ディレクトリ構成）を
#         コードベース側から得る指示の不変コアを持つ
...
#   (D)   `開発原則` に言及する実行時ファイルが、節の不在に対する読み替え禁止を伴うこと
...
# CLAUDE.md に無い項目をコードベース側から得る指示。
DDF_DERIVE='開発原則・技術スタック・ディレクトリ構成は `CLAUDE.md` に書かれないのが既定。'
DDF_NO_HALT='**記載が無いことを理由に停止しない**'
# 開発原則の節の不在に対する読み替え禁止（軽量版。検証者系エージェント）。
...
for f in "${DDF_DISCOVERY_FILES[@]}"; do
  ...
  assert_eq "(A-2) ${f}: CLAUDE.md に無い項目をコードベースから得る指示が在る" "true" \
...
while IFS= read -r f; do
  ...
  if [ "$(has_literal "$f" "$DDF_DERIVE")" = "true" ] \
    || [ "$(has_literal "$f" "$DDF_INTENT")" = "true" ]; then
    continue
  fi
  DDF_BARE="${DDF_BARE}${f} "
