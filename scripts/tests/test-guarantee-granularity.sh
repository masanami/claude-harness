...
gg_core "(G-3) 停止条件が「それ以上割らない」条件として提示されている" \
  '**次の3つのいずれかに当たれば、それ以上割らない**（原子化の停止条件）'
gg_core "(G-3) 停止条件 S1（同時にしか観測できない構成要素）" '**(S1) 同時にしか観測できない構成要素**'
gg_core "(G-3) 停止条件 S2（同一規則の実例）" '**(S2) 同一規則の実例**'
gg_core "(G-3) S2 は規則を1約束とし実例は約束文に列挙する" \
  '**規則を1約束とし、実例は約束文の中で列挙する**'
gg_core "(G-3) S2 は「単文の見出し＋多数の引用」を正常形と認める" \
  '**見出しが単文で引用が多数**なのは S2 の正常な形であり、複合文の証拠ではない'
gg_core "(G-3) 停止条件 S3（条件と結果を切り離さない）" \
  '**(S3) 条件と結果を切り離すと「何が・どういう条件で・どうなる」の1文として成り立たなくなる**'
gg_core "(G-3) 割りすぎの検算がある" '割った断片が**独立に引用できない**'
...
gg_core "(G-3) over-claim 禁止とこぼれの Gaps 回送" \
  '**約束を実担保範囲へ狭め、こぼれた範囲は Gaps へ回す**'
