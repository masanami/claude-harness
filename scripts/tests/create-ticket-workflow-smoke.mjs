...
console.log('=== computeGraphMetrics ===');
{
  ...
  assertEq(
    '空タスク配列: 循環なし・幅0・パス長0',
    { maxParallelWidth: 0, criticalPathLength: 0, hasCycle: false },
    computeGraphMetrics([]),
  );

  // 範囲外インデックス(存在しないタスクへのdepends_on)は防御的に無視される
  const outOfRangeDep = [{ depends_on: [99] }];
  assertEq('範囲外インデックスへの依存は無視され循環扱いにならない', false, computeGraphMetrics(outOfRangeDep).hasCycle);
  assertEq('範囲外インデックスへの依存は無視されlevel0扱い(criticalPathLength=1)', 1, computeGraphMetrics(outOfRangeDep).criticalPathLength);
}
