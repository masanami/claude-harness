...
assert_eq "guaranteesLedgerがnull" "null" "$(jq -r '.guaranteesLedger' <<<"$DOCS_JSON")"
...
assert_eq "guaranteesLedgerがdocs/guarantees.md" "docs/guarantees.md" "$(jq -r '.guaranteesLedger' <<<"$DOCS_JSON")"
...
echo "=== test: fetch_docs_evidence (ADR置き場の検出) ==="
ADR_DIR_FIXTURE="${TMP_ROOT}/adr-project"
mkdir -p "$ADR_DIR_FIXTURE/docs/adr"
: > "$ADR_DIR_FIXTURE/docs/adr/0001-auth-model.md"
fetch_docs_evidence "$ADR_DIR_FIXTURE"
assert_eq "adrDirがdocs/adr" "docs/adr" "$(jq -r '.adrDir' <<<"$DOCS_JSON")"

echo "=== test: fetch_docs_evidence (空のADRディレクトリは未整備扱い) ==="
ADR_EMPTY_FIXTURE="${TMP_ROOT}/adr-empty-project"
mkdir -p "$ADR_EMPTY_FIXTURE/docs/adr"
fetch_docs_evidence "$ADR_EMPTY_FIXTURE"
assert_eq "*.mdが1件も無いADRディレクトリはadrDir=null" "null" "$(jq -r '.adrDir' <<<"$DOCS_JSON")"

echo "=== test: fetch_docs_evidence (ADRディレクトリに.md以外しか無い場合も未整備扱い) ==="
ADR_NON_MD_FIXTURE="${TMP_ROOT}/adr-non-md-project"
mkdir -p "$ADR_NON_MD_FIXTURE/docs/adr"
: > "$ADR_NON_MD_FIXTURE/docs/adr/.gitkeep"
fetch_docs_evidence "$ADR_NON_MD_FIXTURE"
assert_eq ".gitkeepのみのADRディレクトリはadrDir=null" "null" "$(jq -r '.adrDir' <<<"$DOCS_JSON")"
