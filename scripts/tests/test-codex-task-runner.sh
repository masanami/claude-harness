...
echo "=== test: --output-schema の差し替え ==="
CUSTOM_SCHEMA="${WORK_DIR}/custom-schema.json"
cp "${REPO_ROOT}/scripts/schemas/codex-task-result.schema.json" "$CUSTOM_SCHEMA" 2>/dev/null \
  || printf '{"type":"object"}\n' >"$CUSTOM_SCHEMA"
OUT="$(run_runner --mode investigate --repo "$TARGET_REPO" --brief-file "$BRIEF_FILE" --output-schema "$CUSTOM_SCHEMA")"
RC=$?
assert_eq "差し替えschemaでもexit 0" "0" "$RC"
...
NO_CODEX_OUT="$(PATH="${JQ_DIR}:${GIT_DIR}:/usr/bin:/bin" "$RUNNER" \
