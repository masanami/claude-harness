...
`missing_test_ref` が検出するのは「**テスト参照が1本も無い**保証」だけであり、「**約束文が述べている事実の数より引用が少ない**」は検出できない。約束文は自然言語であり、そこに含まれる事実の数を機械的に数える手段が無いためである。したがって:

- **複合文の保証に代表テスト1本だけを引いた台帳は `pass` する**（例: 「検証して作成し、不正入力は 400、存在しない参照は 404 を返す」という4つの約束に、400 のテスト1本）。
- **`status: "pass"` が主張するのは「引用が1本以上あり、それが実在するファイル・テスト名へ解決すること」だけ**である。「保証が1約束に割れていること」「約束の全事実が裏付けられていること」は**主張しない**。呼び出し側は `pass` を「約束が守られている」の根拠にしないこと。

粒度と網羅性の規約の正本は `docs/ai-driven-development-strategy.md` 5.3「保証の粒度」であり、その強制は**裁可（人間）と意味整合監査**（`/guarantee-audit drift` ／ `/promote-verify` Step 5.5 の `guarantee-auditor` の `verify`）が担う。**本スクリプトへ粒度の検査を足すことで解決しない**——約束文の意味を読む処理は決定的スクリプトの責務ではない（同 5.6 のレイヤリング: 決定的に検出できるものは機械で毎ループ、意味の判断が要るものは LLM で低頻度）。

なお、**1保証 = 1約束が守られている台帳に限り、`missing_test_ref` は「裏付けの無い約束」の機械的な近似として働く**。複合文を許すと機械の `pass` と規約の充足が乖離し、その乖離幅は約束文の節数に比例して広がる。原子化はこの乖離を「引用はあるが内容がずれている」場合（＝意味ドリフト）だけに縮める。
