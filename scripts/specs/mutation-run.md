...
- **`test_command` はシェルへ渡さない**（Issue #223）。空白で argv に分解して直接実行するため、シェル構文（`;` `&&` `|` `>` `$(…)` `` ` `` クォート グロブ）は使えず、argv が `scripts/config/command-allowlist.txt` のエントリに一致しないコマンドも実行できない（`bundle exec` 等のラッパーは残りの argv も一覧に載っていることを要求する）。いずれも**テストを実行する前に** exit code 4 で拒否し、stdout に JSON は出さない（注入済みの変異は EXIT トラップの安全網で復元を試みる）。`Bash(claude-harness-run:*)` の allow が、**ランチャーへ直接渡す文字列**で settings.json の deny を迂回する経路にならないようにするための契約（子プロセスや本スクリプト自身が呼ぶ `git` には permission 判定が適用されない点を含め、正本は `docs/script-launcher.md`「6. このランチャーを allow することの意味」）
