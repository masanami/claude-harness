...
- **`test_command` はシェルへ渡さない**（Issue #223）。空白で argv に分解して直接実行するため、シェル構文（`;` `&&` `|` `>` `$(…)` `` ` `` クォート グロブ）は使えず、argv の先頭トークン列が `scripts/config/command-allowlist.txt` に前置一致しないコマンドも実行できない。いずれも**テストを実行する前に** exit code 4 で拒否し、stdout に JSON は出さない（注入済みの変異は EXIT トラップの安全網で復元を試みる）。`Bash(claude-harness-run:*)` の allow が settings.json の deny を迂回する経路にならないようにするための契約であり、背景は `docs/script-launcher.md`「6. このランチャーを allow することの意味」
